...
all_bids = pd.read_excel(open('Bids_energy_33 - copia.xlsx','rb'),sheet_name='T1_best',index_col=0)
...
#%% Function to match an offer
    def matching(bid_type, Setpoint, bid, orderbook_request, orderbook_offer, orderbook_temporary,  matches, Social_Welfare, Flex_procurement):
        ...
        for ID in orderbook.index:
            ...
            #print (orderbook)
            if bid_nature == 'Offer':
                #print (orderbook_request.at[ID,'Quantity'])
                if orderbook['Quantity'].sum() != orderbook_request['Quantity'].sum():
                    #sys.exit("Orderbook has not been updated properly")
                    print ("Orderbook has not been updated properly")
                                                ...
                                                if BB_matches1.empty:
                                                    ...
                                                    if BB_matches2.empty:
                                                        ...
                                                        for m in BB_matches.index:
                                                            ...
                                                            # print (Setpoint.at[BB_matches.at[m,'Time_target'],'Setpoint_P'])
                        ...
                        if Quantity > epsilon: # Line constraints are respected
                            ...
                            if bid_nature == 'Offer':
                                ...
                                if orderbook_request.at[ID,'Temporary'] == 'No':
                                    ...
                                    #print (orderbook_request, orderbook_offer, orderbook_temporary)
...
# new_setpoint = pd.read_excel(open('Setpoint_nodes - copia.xlsx', 'rb'),sheet_name='Nodes33_infeasible',index_col=0) # Baseline injections at each nodes (negative for retrieval)

# for bids in offers_accepted.index:
#     if offers_accepted.at[bids,'Direction'] == 'Up':
#         new_setpoint.at[offers_accepted.at[bids,'Time_target'],offers_accepted.at[bids,'Bus']] += offers_accepted.at[bids,'Quantity']
#     if offers_accepted.at[bids,'Direction'] == 'Down':
#         new_setpoint.at[offers_accepted.at[bids,'Time_target'],offers_accepted.at[bids,'Bus']] -= offers_accepted.at[bids,'Quantity']
