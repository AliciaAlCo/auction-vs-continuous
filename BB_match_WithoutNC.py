...
def BB_match_WithoutNC(setpoint,orderbook_temporary,offer_index,offer_quantity, lines, nodes, direction, offer_bus, offer_price, offer_block):
    ...
    if orderbook_temporary.shape[0]==1:
     ...
     #%% Optimization problem 
        def req_selection():
            ...
            m.pru_limit = pyo.Constraint(m.RU, rule = pru_lim) # Flexibility limits for requests and offers
            m.prd_limit = pyo.Constraint(m.RD, rule = prd_lim) # Flexibility limits for requests and offers
